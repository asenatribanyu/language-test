        ...
        <div
            ...
            <div class="mt-5">
                ...
                <input type="text" id="base-input" disabled
                    value="{{ $ept_opens ? $ept_opens->exam->title : $toeic_opens->exam->title }}"
            ...
            <div class="my-5">
                ...
                <input type="text" id="base-input" disabled
                    value="{{ $ept_opens->date == '0' ? 'First Date: ' . \Carbon\Carbon::parse($ept_opens->exam->first_date)->translatedFormat('j F Y') : ($ept_opens->date == '2' ? 'Second Date ' . \Carbon\Carbon::parse($ept_opens->exam->second_date)->translatedFormat('j F Y') : 'Third Date: ' . \Carbon\Carbon::parse($ept_opens->exam->third_date)->translatedFormat('j F Y')) }}"
                    class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
            </div>
            <div class="mt-5">
            ...
            <div>
                <section class="mt-5">
                    <table id="myTable" class="display w-full text-sm text-left text-gray-500 dark:text-gray-400">
                        ...
                        @if ($ept_opens)
                            <tbody>
                                @foreach ($ept_opens->exam->enroll as $enroll)
                                                ...
                                                <div class="relative p-4 w-full max-w-md max-h-full">
                                                    <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                                        ...
                                                        <div class="p-4 md:p-5 text-center">
                                                            ...
                                                            <form
                                                                action="/dashboard/waiting-area/jadwal/{{ $enroll->id }}"
                                                                method="POST">
                                                                @method('put')
                                                                @csrf
                        ...
                        @else
                            <tbody>
                                @foreach ($toeic_opens->exam->enroll as $enroll)
                                    <tr class="border-b bg-gray-50 dark:bg-gray-800 dark:border-gray-700">
                                        <th scope="row"
                                            class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                                            {{ $loop->iteration }}
